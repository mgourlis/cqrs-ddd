...
async def test_create_compound_index_creates_single_field_index(mock_connection):
    ...
    index_name = await create_compound_index(connection, "test_db", "test_collection", [("field1", 1)])
...
async def test_create_compound_index_with_multiple_fields(mock_connection):
    ...
    indexes = [idx async for idx in coll.list_indexes()]
    index_names = [idx.get("name") for idx in indexes if idx.get("name")]
    assert "field1_1_field2_-1" in index_names or index_name in index_names or len(indexes) >= 2
...
async def test_create_compound_index_with_unique_constraint(mock_connection):
    ...
    index_name = await create_compound_index(connection, "test_db", "test_collection", [("email", 1)], unique=True)
...
async def test_create_compound_index_with_custom_name(mock_connection):
    ...
    index_name = await create_compound_index(
        connection, "test_db", "test_collection", [("field1", 1), ("field2", -1)], name="custom_index"
    )
...
async def test_create_ttl_index_creates_time_to_live_index(mock_connection):
    ...
    index_name = await create_ttl_index(connection, "test_db", "test_collection", "created_at", expire_after_seconds=3600)
    ...
    assert any("created_at" in idx["name"] for idx in indexes) or index_name in index_names
...
async def test_create_text_index_creates_text_index(mock_connection):
    ...
    index_name = await create_text_index(connection, "test_db", "test_collection", [("content", "text")])
    ...
    indexes = [idx async for idx in coll.list_indexes()]
    index_names = [idx.get("name") for idx in indexes if idx.get("name")]
    assert "content_text" in index_names or index_name in index_names or len(indexes) >= 2
