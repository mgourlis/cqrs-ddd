...
# Test Models
class TestProjectionModel(BaseModel):
...
class TestMongoProjectionStoreUpsert:
    ...
    async def test_upsert_pydantic_model(self, projection_store):
        ...
        model = TestProjectionModel(name="Model Test", value=100)
        doc_id = model.id
    ...
    async def test_upsert_with_custom_id_field(self, projection_store):
        ...
        doc = await coll.find_one({"_id": doc_id})
        assert doc is not None
        assert "custom_id" not in doc  # Should be moved to _id
...
class TestMongoProjectionStoreUpsertBatch:
    ...
    async def test_upsert_batch_with_mixed_types(self, projection_store):
        ...
        model_doc = TestProjectionModel(name="Model Doc", value=2)
