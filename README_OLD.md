...
3. **Polyglot persistence** — Write side (Postgres/SQLAlchemy) is separate from read side (Mongo). Sync via projection workers: event store → ProjectionWorker → IProjectionWriter + IProjectionPositionStore (see [docs/projection_writer.md](docs/projection_writer.md), [docs/projection_position_tracking.md](docs/projection_position_tracking.md)).
